...
part 'mixin/create_questionnaire_mixin.dart';

class CreateQuestionnairePage extends StatefulWidget {
  const CreateQuestionnairePage({super.key});

  @override
  State<CreateQuestionnairePage> createState() => _CreateQuestionnairePageState();
}

class _CreateQuestionnairePageState extends State<CreateQuestionnairePage> with CreateQuestionnaireMixin {
  @override
  Widget build(BuildContext context) {
    return BlocConsumer<QuestionnaireBloc, QuestionnaireState>(
      listenWhen: (p, c) => p.createQuestionnaireApiStatus != c.createQuestionnaireApiStatus,
      listener: (context, state) {
        if (state.createQuestionnaireApiStatus.isSuccess) {
          context.showFloatingSnackBar(
            message: 'profile_created_successfully'.tr(),
            backgroundColor: Colors.green,
          );
          context.pushReplacement(Routes.imageUpload, extra: state.photoLimit);
        }
        if (state.createQuestionnaireApiStatus.isError) {
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: CustomTextField(
                              ...
                              nextFocus: healthyStatusFocusNode,
                              autoValidateMode: autoValidate ? AutovalidateMode.always : AutovalidateMode.disabled,
                              validator: (v) {
                                if (v?.isNotEmpty ?? false) {
                                  return null;
                    ...
                    Padding(
                      ...
                      child: CustomTextField(
                        ...
                        autoValidateMode: autoValidate ? AutovalidateMode.always : AutovalidateMode.disabled,
                        validator: (v) {
                          if (v?.isEmpty ?? false) return 'profession_hint'.tr();
                          return null;
                        },
                      ),
                    ),
                    ...
                    MoreSelectWidget(
                      isRequired: true,
                      errorText: (state.childPlan?.isEmpty ?? false) && autoValidate ? 'childPlan'.tr() : '',
                      title: 'childPlan'.tr(),
                      margin: AppUtils.kPaddingHorizontal16,
                      onClose: (String v) {
                        context.read<QuestionnaireBloc>().add(UnSelectGenderTagsProfileEvent(v));
                    ...
                    AnimatedContainer(
                      height: !((state.numberOfChildren ?? 0) <= 0) ? 80.h : 0,
                      width: 1.sw,
                      duration: Duration(seconds: 1),
                      child: SingleChildScrollView(
                        physics: NeverScrollableScrollPhysics(),
                      ...
                      child: AppButton(
                        ...
                        onPressed: () {
                          ...
                          if ((formKey.currentState?.validate() ?? false) &&
                            ...
                            context.showFloatingSnackBar(message: 'please_complete_this_fieldd'.tr());
                        ...
                        title: 'save'.tr(),
                        child: state.createQuestionnaireApiStatus.isLoading
